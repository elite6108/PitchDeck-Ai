...
type DesignStyle = 'bold' | 'modern' | 'classic' | 'minimal' | 'creative' | undefined;
...
const DeckExport: React.FC<DeckExportProps> = ({ deck, onClose }) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      try {
        // Get all slide elements after they've been rendered
        const slideElements = slidesContainerRef.current.querySelectorAll('.slide-wrapper');
        
        if (slideElements.length === 0) {
          console.warn('No slide elements found, looking for alternative selectors');
          // Try alternative selectors if the expected one doesn't work
          const alternativeElements = slidesContainerRef.current.querySelectorAll('.slide-for-export');
        ...
        for (let i = 0; i < slideElements.length; i++) {
          ...
          try {
            ...
            const canvas = await html2canvas(slideElement, {
              scale: 0.75, // Scale down a bit to use less memory
              useCORS: true,
              allowTaint: true,
              ...
              onclone: (document) => {
                ...
                const clonedElement = document.querySelector('.slide-wrapper') as HTMLElement;
                if (clonedElement) {
  ...
  const applyAiStyling = async () => {
    try {
      ...
      const recommendedTheme: ColorTheme = themeMap[industryKey] || 'blue';
      console.log('Using industry key:', industryKey, 'Theme:', recommendedTheme);
      
      // Direct background image URLs - no CDN or query params to cause issues
      const backgroundImages: Record<string, string[]> = {
        'technology': [
          'https://images.unsplash.com/photo-1581094794329-c8112a89f12e',
      ...
      // Get relevant background images for the industry
      let industryBackgrounds = backgroundImages[industryKey] || 
                               backgroundImages['technology'] || 
                               backgroundImages['default'];
      
      console.log('Selected background images:', industryBackgrounds);
      
      // Ensure we have valid background images
      if (!industryBackgrounds || industryBackgrounds.length === 0) {
        console.error('No background images available');
        // Use fallback images if industry backgrounds are not available
        industryBackgrounds = [
          'https://images.unsplash.com/photo-1579546929518-9e396f3cc809',
          'https://images.unsplash.com/photo-1557683316-973673baf926'
        ];
      ...
      if (aiStyledDeck.slides) {
        aiStyledDeck.slides = aiStyledDeck.slides.map((slide, index) => {
          ...
          styledSlide.content = {
            ...
            design_style: analysis.recommendedStyle === 'corporate' ? 'modern' : 'minimal',
            font_style: analysis.businessTone === 'professional' ? 'sans-serif' : 'serif'
          };
          
          // Apply background images to specific slide types
          if (styledSlide.slide_type === 'cover') {
            // Apply a background image to the cover slide
            styledSlide.content.background_image = industryBackgrounds[0];
            // Log what we're setting for debugging
            console.log('Setting cover slide background:', industryBackgrounds[0]);
          } else if (styledSlide.slide_type === 'image' || styledSlide.slide_type === 'media' || index % 3 === 0) {
            // Apply background image to certain other slides for visual interest
            styledSlide.content.background_image = industryBackgrounds[1];
            console.log(`Setting slide ${index} background:`, industryBackgrounds[1]);
          }
    ...
    <div className="z-10 absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        ...
        <div className="flex-grow overflow-auto p-6">
          ...
          {!useAiStyling && (
            <div className="mb-3 sm:mb-4 md:mb-6 border border-gray-200 rounded-lg overflow-hidden">
              ...
              {showThemeSelector && (
                <ThemeSelector 
                  currentTheme={selectedTheme || 'blue'}
      ...
      <div className="absolute left-[-9999px] top-[-9999px] overflow-hidden">
        <div ref={slidesContainerRef} className="export-slides-container">
          ...
          {(exporting || true) && updatedDeck.slides?.map((slide, index) => (
